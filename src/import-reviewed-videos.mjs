...
import { normalizeReviewedItem } from "./editorial.mjs";
...
const videos = reviewed.map((sourceItem) => {
  const item = normalizeReviewedItem(sourceItem);
  return {
    id: item.videoId,
    // Existing public URLs remain stable even when legacy display titles are cleaned.
    slug: sourceItem.slug || slugify(sourceItem.proposedTitle || sourceItem.sourceTitle),
